...
const viewTodos = () => {
  const table = new Table({
    ...
    style: { "padding-left": 1, "padding-right": 1 }, // Adjust padding as needed
    head: ["#", "Todo"],
...
const handleSpinner: (start: string, stop: string) => Promise<void> = async (
  start,
  stop
) => {
...
const TodoFunc = async () => {
  ...
  if (res.select == "Add") {
    ...
    await handleSpinner(
      "Adding Todo...",
      chalk.green("Todo Added Successfully.")
    );
  }
  if (res.select == "Update") {
    ...
    if (todos.length !== 0) {
      ...
      await handleSpinner(
        "Updating Todo...",
        chalk.green("Todo Updated Successfully.")
      );
    } else {
  ...
  if (res.select == "Delete") {
    ...
    if (todos.length !== 0) {
      ...
      await handleSpinner(
        "Deleting Todo...",
        chalk.green("Todo Deleted Successfully.")
      );
    } else {
  ...
  if (res.select == "Exit") {
    handleSpinner("Exiting...", chalk.green("ATM Machine Closed."));
    exit = false;
  }
};
await welcome();
